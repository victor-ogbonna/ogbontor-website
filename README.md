...
```
index.html      Home — hero carousel, programme rail, portfolio companies, sponsorship
register.html   Conference + bootcamp registration form (posts to your Google Sheet)
...
and a **hackathon** closes the bootcamp with a demo day. That order is stated once, in `EVENTS` and
`PROGRAMME_LINE` in `tools/build.py`, and rendered by `programme_rail()` on both the home page and
`register.html` — so the two can never drift apart. The registration form asks which acts the person
is attending (at least one required — enforced in JS, since HTML cannot mark a checkbox group
required).

Change the order or the wording in `EVENTS` and both pages follow.
...
- **Motion graphics** — the cyber/mechatronic set pieces (circuit backdrop, robot arm, gear train,
  chip core, oscilloscope, radar) are generated by the `mg_*` helpers in `tools/build.py`. They emit
  plain `aria-hidden` SVG; all the movement is CSS keyframes in the *Futuristic layer* block at the
  end of `styles.css`, with every rest pose on `0%/100%` so the single `prefers-reduced-motion` rule
  at the top of that file freezes the whole set on a sensible frame. No JavaScript, no dependencies.
